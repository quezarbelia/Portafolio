...
// Interactive Constellation Data Mesh Particle System
...
// Live Terminal Pipeline Activity Simulation
function initTerminalSimulation() {
  const terminal = document.querySelector('section .space-y-2.font-mono');
  if (!terminal) return;
  
  const liveLine = document.createElement('div');
  liveLine.className = 'text-emerald-400/90 pt-2 flex items-center gap-2 font-mono text-[11px]';
  liveLine.innerHTML = `
    <span class="inline-block w-2 h-2 rounded-full bg-emerald-400 animate-ping"></span>
    <span>Live Daemon ETL: <span id="terminal-records-counter" class="text-white font-bold tracking-wide">14,820</span> registros procesados ACID</span>
    <span class="animate-pulse text-cyan-400 font-bold ml-1">▋</span>
  `;
  terminal.appendChild(liveLine);

  let count = 14820;
  setInterval(() => {
    count += Math.floor(Math.random() * 14) + 4;
    const counterEl = document.getElementById('terminal-records-counter');
    if (counterEl) {
      counterEl.textContent = count.toLocaleString('es-MX');
    }
  }, 2400);
}
